function LocationPinIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0 mt-0.5" aria-hidden="true">
      <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" />
      <circle cx="12" cy="10" r="3" />
    </svg>
  );
}

function PhoneIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0" aria-hidden="true">
      <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.6 1.27h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.91a16 16 0 0 0 6 6l.91-.91a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7a2 2 0 0 1 1.72 2.03z" />
    </svg>
  );
}

function WhatsAppIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" className="shrink-0" aria-hidden="true">
      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413z" />
    </svg>
  );
}

export default function FindUsSection() {
  return (
    <section className="flex flex-col lg:flex-row min-h-[420px]">
      {/* Left — info panel */}
      <div className="lg:w-1/2 bg-[#2B7FC1] text-white px-8 sm:px-12 lg:px-16 py-12 flex flex-col gap-6">
        {/* Label */}
        <p className="text-xs font-semibold tracking-[0.2em] uppercase text-white/80">
          Find Us
        </p>

        {/* Main clinic */}
        <div className="flex flex-col gap-4">
          <h2 className="text-2xl sm:text-3xl font-bold">HMI Medical Centre</h2>

          <div className="flex flex-col gap-3 text-sm text-white/90">
            <div className="flex items-start gap-3">
              <LocationPinIcon />
              <span>12 Farrer Park Station Road, #05-01, Singapore 217565</span>
            </div>
            <div className="flex items-center gap-3">
              <PhoneIcon />
              <a href="tel:+6563226333" className="hover:text-white transition-colors">
                +65 6322 6333
              </a>
            </div>
            <div className="flex items-center gap-3">
              <WhatsAppIcon />
              <a
                href="https://wa.me/6596552101"
                target="_blank"
                rel="noopener noreferrer"
                className="hover:text-white transition-colors"
              >
                +65 9655 2101
              </a>
            </div>
          </div>

          <div className="flex flex-col gap-1.5 text-sm text-white/90">
            <p>
              <span className="font-semibold text-white">General enquiries: </span>
              <a
                href="mailto:info.hmimedicalcentre@hmimedical.com"
                className="hover:text-white transition-colors break-all"
              >
                info.hmimedicalcentre@hmimedical.com
              </a>
            </p>
            <p>
              <span className="font-semibold text-white">Feedback: </span>
              <a
                href="mailto:concierge.hmimedicalcentre@hmimedical.com"
                className="hover:text-white transition-colors break-all"
              >
                concierge.hmimedicalcentre@hmimedical.com
              </a>
            </p>
          </div>
        </div>

        {/* Divider */}
        <div className="h-px bg-white/20" />

        {/* Satellite clinic */}
        <div className="flex flex-col gap-3">
          <h3 className="text-lg font-bold">Satellite Clinic</h3>
          <div className="flex items-start gap-3 text-sm text-white/90">
            <LocationPinIcon />
            <span>
              3 Mount Elizabeth, #12-14, Mount Elizabeth Medical Centre,
              Singapore 228510
            </span>
          </div>
        </div>
      </div>

      {/* Right — Google Maps embed */}
      <div className="lg:w-1/2 min-h-[320px] lg:min-h-0">
        <iframe
          title="HMI Medical Centre Location"
          className="w-full h-full border-0 min-h-[320px]"
          loading="lazy"
          allowFullScreen
          referrerPolicy="no-referrer-when-downgrade"
          src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3988.7474095445366!2d103.85406!3d1.31213!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x31da19f4bc4f2e77%3A0xa4c9e80f87fea0!2s12%20Farrer%20Park%20Station%20Rd%2C%20%2305-01%2C%20Singapore%20217565!5e0!3m2!1sen!2ssg!4v1699000000000!5m2!1sen!2ssg"
        />
      </div>
    </section>
  );
}
